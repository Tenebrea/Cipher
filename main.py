...
class MainWindow(QMainWindow):
    ...
    def encrypt_text(self):
        ...
        match self.cipher:
            case "vigenere":
                self.text = vigenere_cipher(self.text, self.key, self.encrypt)
            case "caesar":
                try:
                    self.key = int(self.input_key_caesar.text())
                    self.text = caesars_cipher(
                        self.text, self.key, self.encrypt)
                except:
                    self.key = ""
                    self.text = ""
                    return
            case "base64":
                self.text = base64_encoder(self.text, self.encrypt)
            case "verman":
